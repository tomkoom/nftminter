...
const Contacts = ({ nftminter }) => {
	async function doInsert() {
		...
		nftminter.insert(name, add1, add2, email, parseInt(phone, 10));
	}

	async function lookup() {
		let name = document.getElementById("lookupName").value;
		nftminter.lookup(name).then((opt_entry) => {
			let entry;
	...
	return (
		<div className="new-entry">
			...
			<div>
				Lookup name: <input id="lookupName" style={{ lineHeight: "20px" }}></input>
